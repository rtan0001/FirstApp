...
xvfb-run -a behave --junit -t ~wip -D url_to_test=ugm.corp-poc.aws.monash.edu features/smoke.feature

# And because the packages are local, the behave command is not in the path
# Using the fully qualified path fixes the issue
xvfb-run -a behave --junit -t ~wip -D url_to_test=ugm.corp-poc.aws.monash.edu
